...
## 5. 別の端末・デスクトップアプリで続きをやる

iPhone やブラウザで始めたセッションの続きを、PC で引き継ぐ方法。

### 方法A: 会話ごと引き継ぐ(ターミナル)

`--teleport` を使うと、会話履歴が丸ごとローカルのターミナルに読み込まれる。
ブランチも自動で fetch・checkout される。
...
セッション ID は claude.ai/code のセッション URL
(`https://claude.ai/code/session_XXXX` の `session_XXXX` 部分)。
引数なしで `claude --teleport` を実行すると一覧から選べる。

**条件が3つある。**

- 同じ claude.ai アカウントでログインしていること
- そのリポジトリのクローンの中で実行すること(別リポジトリだとエラー)
- 作業ツリーがクリーンなこと(未コミットの変更があると stash を促される)

引き継いだ後のターミナル側の作業はローカル専用になり、クラウド側のセッションには
反映されない。コピーが渡される形。

### 方法B: デスクトップアプリで新規セッション

**デスクトップアプリには、クラウドセッションをローカルに引き取る機能がない。**
アプリの「Continue in」メニューは ローカル → Web の一方向で、逆はできない。
`--teleport` はターミナル(CLI)専用。

デスクトップアプリで作業したい場合は、新しいローカルセッションを開いて
リポジトリの成果物を読ませる形になる。会話履歴は引き継がれないが、
成果物がリポジトリに入っていれば実害は小さい。

1. デスクトップアプリ → **Code** タブ → 「**+ New session**」
2. **Environment** を `Local`、**Project folder** にクローンしたフォルダを指定
3. 作業ブランチに切り替える(成果物が main にない場合は必須)

```bash
git checkout claude/xxxxx
```

4. 最初のメッセージで、読んでほしいファイルと今どこまで進んだかを伝える

```
XXXX.md を読んで。(状況)。続きを手伝って。
```

### 使い分け

| | 方法A(teleport) | 方法B(デスクトップ新規) |
|---|---|---|
| 会話履歴 | そのまま引き継がれる | 引き継がれない(成果物で代替) |
| 使う場所 | ターミナル | デスクトップアプリ |
| PC 上の別フォルダを触る | 可 | 可(Project folder に指定) |

**引き継ぎを楽にするコツ**: 会話の結論を毎回リポジトリの Markdown に落としておくと、
方法B でも困らない。作業ブランチではなく main に入れておけば
`git checkout` も不要になる。
